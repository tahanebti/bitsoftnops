...
public class SpecificationsBuilder<T> {
    ...
    public Specification<T> with(String query) {
         ...
         Optional<String> predicateFlagOpt = Optional.empty();        
 		...
 		if(splitOperation.isPresent()) {
         	...
         	predicateFlagOpt = Optional.ofNullable(splitOperation.get().contains("&") ? "AND" : splitOperation.get().contains("|") ? "OR" : null);
         }
 		
        
         String[] searchQueries = FilterOperation.splitSearchOperations(query, predicateFlagOpt.orElse(null));
         List<SearchCriteria> params = parse(searchQueries, predicateFlagOpt.orElse(null));
    ...
    public List<SearchCriteria> parse(String[] search, String isOrPredicate) {
        ...
        for (String s : search) {
            ...
            if (matcher.find()) {
             	...
             	Optional.ofNullable(isOrPredicate).ifPresent(section::setIsOrPredicate);
    ...
    private SearchCriteria buildCriteriaFromSection(SearchSection section) {
        ...
        SearchCriteria.SearchCriteriaBuilder builder = SearchCriteria.builder()
        	  ...
        	  Optional.ofNullable(section.getIsOrPredicate()).ifPresent(builder::dataOption);	
        	  Optional.ofNullable(section.getJoinEntity()).ifPresent(builder::joinEntity);
